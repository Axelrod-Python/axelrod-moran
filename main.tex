...
\begin{abstract}
    In this paper an extensive numerical
    study of the evolutionary dynamics of the iterated prisoner's dilemma
    are presented. Fixation probabilities for Moran processes are obtained
    for 164 different strategies including classics such as TitForTat, zero
...
The top strategies are given in Tables~\ref{tbl:top_invade}.
...
is functionally very similar to its deterministic cousin Evolved Looker Up. Apart
from TF3 in \(N=3\), the finite state machines trained specifically for Moran
processes do not appear in the top 5 while strategies trained for tournaments do.
This is due to the nature of invasion: most of the opponents will initially be of
different types. The next section will consider the converse situation.

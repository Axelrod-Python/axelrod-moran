...
\begin{abstract}
    ...
    to maximize total payoff while the best resistors invoke handshake machanisms.
...
and insighed contained in this paper would be difficult to derive analytically.
...
\begin{enumerate}
 ...
 \item Strategies evolved to maximize their total payout can be strong invaders
...
probability with which a given strategy takes over a the population is called the
...
It is also noted that TF1, TF2 and TF3 all perform well, other finite state
machines include Prober 3 and Prober 4 which are described in \cite{prison}.
This is also the \(N\) for which a zero determinant strategy does appear in the
top 10\% ranking strategies: ZD-extort-4. The performance of zero determinant
strategies will be examined more closely in Section~\ref{sec:population_size}.
...
maximise it's utility and does perform well.
Apart from TF3, the finite state machines trained specifically for
Moran processes do not appear in the top 5: whereas strategies trained for
...
There are many opportunities build on this work. In particular, an analysis

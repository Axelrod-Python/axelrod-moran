...
\cite{Press2012} even manipulate players through extortion.
...
strategies. Not all strategies have been considered for this study: same use
knowledge of the game such as the number of turns and others have a high
...
The library can conduct matches, tournaments and population
...
\begin{enumerate}
 ...
 \item Complex strategies can be effective, and in fact can naturally evolve
     through evolutionary processes to outperform intelligently designed
     strategies.
 \item Strong resistors specifically evolve or have a handshake mechanism. This
     has the potential to offer insight in to organisms such as antibacterial
     resistant bacteria \cite{Davies2010}.
\end{enumerate}
...
given in Section~\ref{sec:validation}.
...
optimisation algorithm, all these strategies are trained with an evolutionary
algorithm that perturbs strategy parameters and optimizes the mean total score
against all other opponents \cite{affenzeller2009genetic}. Variation is
...
\cite{axelrodproject} and a detailed description of the performance
 of these strategies in IPD
tournaments will be described in upcoming manuscript.
...
FSM 1, 2, and 3 (TF1 - TF3). These are based on finite state machines of 8, 16,
and 16 states respectively (see Figures \ref{}).
...
product of training with a Moran objective. Interestingly it has no handshake
mechanism, this could be due to the fact that it is trained in a noisy
environment.
...
taken
from a large number of match outcomes for the pairs of players for use in
...
This is described in Figure~\ref{alg:psuedocode}.
% TODO Include pseudo code
...
(\ref{equ:payoff_matrix}).

All data generated for this validation exercise can be found
at \cite{data}.

A detailed analysis of the 164 strategies considered,
using direct Moran processes will be shown in the next Section.
...
\begin{enumerate}
    \item The top strategy is the Collective Strategy (CS) which has a simple
        ...
        rounds. This strategy was specifically designed for
        evolutionary processes \cite{Li2009}.
...
source code of PSO Gambler shows that it makes stochastic decisions with extreme
probabilities: almost always acting as expected. Apart from TF1 in \(N=3\), the
finite state machines trained specifically for Moran processes do not appear in
the top 5: whereas strategies trained for tournament do. This is due to the
nature of invasion: most of the opponents will initially be different
...
\(x_{N-1}\): TF2 and TF3. These two strategies are with CS the strongest
resistors. They all have handshakes, and whilst the handshake that CS and
Handshake (which ranks highly for the smaller values of \(N\)) have was
...
first defection, possibly because the training strategy collection contains a
significant portion of strategies such as Grudger and Fool Me Once that
retaliate harshly by defecting for the remainder of the match if the opponent
has more than a small number of cumulative defections. Paradoxically it is
necessary to defect (as a signal) in order to achieve mutual cooperation with
opponents using the same strategy but not with other opponents.
...
This is confirmed by the performance of the Zero determinant strategies, whilst
some do rank relatively highly for \(N=2\) (ZD-extort-4 has tank 16) this rank
...
Figure~\ref{fig:correlation_coeficients} show the correlation coefficients
...
was not only identify for CS (an intelligently designed strategy) but also for
two FSM strategies (TF2 and TF3) specifically trained through an evolutionary
